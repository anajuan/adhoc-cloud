...
"cluster")
	...
	#data_ini=`date +%s`
	data_ini=`date +%s.%3N`
	fileName="data/data-awsa1EtcdCluster-$data_ini-$clusterSize.dat"
	...
	for key in "${!ary[@]}"; do 
		...
		ELAPSED_TIME=$(awk -v v1=$data_end -v v2=$data_ini 'BEGIN {print v1-v2}')
		ELAPSED_TIME_INS=$(awk -v v1=$data_end -v v2=$data_iniIns 'BEGIN {print v1-v2}')
	...
	### Step 5: save memory usage on each node

	for key in "${!ary[@]}"; do
		node=${ary[$key]}
                ssh ubuntu@$node "export PATH=$PATH:/snap/bin/docker;docker stats --no-stream">>$fileName
	done
	;;
*)
	echo "k ase?"
